...
export class SaucesService {
  ...
  getSauces() {
    this.http
      .get<Sauce[]>('https://back-piquante-sj89.vercel.app/api/sauces')
      .pipe(
  ...
  getSauceById(id: string) {
    return this.http
      .get<Sauce>('https://back-piquante-sj89.vercel.app/api/sauces/' + id)
  ...
  likeSauce(id: string, like: boolean) {
    return this.http
      .post<{ message: string }>(
        'https://back-piquante-sj89.vercel.app/api/sauces/' + id + '/like',
  ...
  createSauce(sauce: Sauce, image: File) {
    ...
    return this.http
      .post<{ message: string }>(
        'https://back-piquante-sj89.vercel.app/api/sauces',
        formData
      )
      .pipe(catchError((error) => throwError(error.error.message)));
  }
  ...
  modifySauce(id: string, sauce: Sauce, image: string | File) {
    if (typeof image === 'string') {
      return this.http
        .put<{ message: string }>(
          'https://back-piquante-sj89.vercel.app/api/sauces/' + id,
          sauce
        )
        .pipe(catchError((error) => throwError(error.error.message)));
    } else {
